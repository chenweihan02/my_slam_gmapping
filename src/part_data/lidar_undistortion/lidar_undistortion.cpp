                                            ...
                                            tf::TransformListener* tf_)
{
    ...
    //toSec()　获取的是自1970.1.1现在时刻的秒数
    double start_time = startTime.toSec() * 1000 * 1000;    //*1000*1000转为时间单位为us
    ...
    for (int i = 0; i < beamNumber; i ++ )  //每一个激光束
    {
        ...
        if (mid_time - start_time > interpolation_time_duration || (i == beamNumber - 1))
        {
            ...
            //对本分段的激光点进行运动畸变的去除
            lidarMotionCalibration(frame_base_pose,     //对于每一帧激光雷达数据，传入参数基准坐标系是不变的
...
bool LidarMotionCalibrator::getLaserPose(tf::Stamped<tf::Pose>& odom_pose, ros::Time dt, tf::TransformListener* tf_)
{
    ...
    //定义一个　tf::Stamped对象，构造函数的入口参数(const T& input, const ros::Time& timestamp, const std::string & frame_id)
                                                    ...
                                                    int startIndex, int& beam_number)
{
    ...
    for (int i = 0; i < beam_number; i ++ )
    {
        ...
        if (std::isinf(ranges[startIndex + i]) == false)
        {
            ...
            //该分段中的激光点，变换的里程计坐标系下的坐标（这里用插值的激光雷达坐标系近似上面哪个真实存在的激光雷达坐标系，因为知道，可以进行计算）
