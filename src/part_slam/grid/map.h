...
namespace GMapping
{
    ...
    */
    ...
    struct PointAccumulator
    {
        ...
        //计算栅格被击中坐标累计值的平均值
        inline Point mean() const {return 1.0/n*Point(acc.x, acc.y);}
        //返回该栅格被占用的概率，范围是　-1(没有访问过) [0, 1]
        inline operator double() const {return visits?(double)n * 1 / (double)visits : -1;} // n/v
        //更新该栅格成员变量
    ...
    */
    ...
    /**
     * 在gmapping使用过程中，cell为PointAccumulator Storage为HierarchicalArray2D<PointAccumulator>
     * 
    ...
    */
    ...
    class Map
    {
        public:
            ...
            //判断某一物理位置或者栅格坐标是否在地图里面　引用的是模板类m_storage(实际上是HierarchicalArray2D)中的函数
