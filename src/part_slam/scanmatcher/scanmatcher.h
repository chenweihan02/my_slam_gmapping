...
namespace GMapping
{
    ...
    /** 
     * 《概率机器人》likehood_field_range_finder_model
     * 输入当前地图和当前经过运动模型更新的激光雷达位姿p，以及激光雷达数据
     * 遍历若干激光束，求当前激光雷达位姿p与地图的匹配得分，越匹配，位姿得分越高
     * 匹配的方式就是找到，击中点和九宫格中的点的差距最小的点，差距越小，得分越高，越匹配，并且遍历若干束激光束局，计算累计得分
     */
    inline double ScanMatcher::score(const ScanMatcherMap &map, const OrientedPoint &p, const double *readings) const
    {
        ...
        unsigned int skip = 0;
        double freeDelta = map.getDelta() * m_freeCellRatio; // 默认0.05x1.414距离，表示斜着一个栅格的差距

        // 枚举所有的激光束，有时为了提高计算速度，不需要对所有激光数据进行计算
        for (const double *r = readings; r < readings + m_laserBeams; r++, angle++)
        {
            ...
            for (int xx = -m_kernelSize; xx <= m_kernelSize; xx++)
            {
                for (int yy = -m_kernelSize; yy <= m_kernelSize; yy++)
                {
                    ...
                    //(double)cell 使用操作符重载，表示被占用的概率
                    if (((double)cell) > m_fullnessThreshold && ((double)fcell) < m_fullnessThreshold)
                    {
                        /*
                            这里要理解一个事情，world2map会将物理坐标小于地图分辨率容差的物理坐标分到一个栅格
                            所以才累计栅格被击中的物理位置求平均，也就是cell.mean()，也就是历史栅格物理位姿的平均
                            通过mu表示击中点和九宫格中的点的差距
                            我们寻找最小差距的那一个点
                        */
            ...
            /**
             * score的计算公式　exp(-d^2 / sigma) 这里的sigma表示方差
             * m_gaussianSigma默认为0.5
             */
            if (found)
            {
